...
| 4 | B | Norma debe vincular Google en Perfil antes de cualquier reinstalación; si no, el uid anónimo nuevo pierde OWNER y el push queda en `PERMISSION_DENIED`. La mitad de Fase 2 está hecha (`19fafeb`): Perfil muestra el estado de identidad y avisa de forma persistente en sesión anónima. Queda para la Fase 11 que Norma lo haga de verdad antes del cutover. | memoria `finance-app-consolidacion-2026-07-09` | 2, 11 |
...
| 8 | F | El push de HOUSEHOLD no está cableado en `SyncManager`: ediciones del hogar quedan locales. Resuelto en la Fase 2 (`d0c59be`, `ca02f6a`): kind HOUSEHOLD, repo remoto con el espejo completo de la entidad, mapper y listener del documento del hogar. De paso se unificó el id, porque el wizard acuñaba uno en Room y `createHousehold` acuñaba otro: el hogar local y el de la nube nunca fueron el mismo documento. | `data/local/dao/HouseholdDao.kt:33-36`; kinds del `SyncManager` = CATEGORY, EXPENSE, INCOME, INSTALLMENT, LOAN, MEMBER, RECURRENCE, SAVINGS, TRANSFER, WALLET | 2 |
| 9 | F | Lápidas verificadas solo para gastos; el resto de entidades puede resucitar tras un offline prolongado. Deriva de saldo multi-dispositivo (snapshot absoluto con LWW). Resuelto en la Fase 2 (`12be51b`, `dbaa908`): helper compartido de lápidas, `onRemoved` en las cuatro colecciones que las ignoraban, y ancla explícita del saldo con aviso de divergencia y conciliación asistida. Se conservó el snapshot; el rediseño a deltas idempotentes se descartó por coste y riesgo. | memoria `consolidacion-2026-07-09` | 2 |
| 10 | F | El rol Colaborador (MEMBER) lee todo el ledger por API: la regla comodín concede `read` a cualquier miembro sobre `households/{hid}/**`. Resuelto en la Fase 2 (`d740eaf`): lectura por colección, sin comodín final, desplegado por Rules API el 2026-09-07 (ruleset `d9623916`) y probado con 60 casos contra el emulador de reglas. | `firestore.rules:129-137` | 2 |
| 11 | F | `statement_import` es local-only por diseño: el checklist "Estados del mes" no converge entre dispositivos. Resuelto en la Fase 2 (`ff0efb2`): kind STATEMENT y migración v21. `payload_json` no viaja, porque el checklist y los recordatorios solo necesitan la cabecera. | `data/local/entity/StatementImportEntity.kt:16` | 2 |
| 12 | F | Un grupo recién creado nace sin cuentas; la siembra de categorías por defecto ya existe pero no está verificada en el flujo multi-hogar. Resuelto en la Fase 2 (`c5116bd`): `switchActiveHousehold` siembra las categorías y el vacío de Cuentas ofrece las cuentas por defecto. | memoria `bugs-2026-07-10`; `data/local/DefaultCategoryCatalog.kt` | 2 |
| 13 | F | Sin exportación ni respaldo visible: RF-34 (reporte PDF/XLSX), RF-91 (XLSX con el layout del Excel), RF-92 (CSV) no existen; el comentario de Perfil lo deja como "se añadirá". Sin cierre manual de quincena (RF-32). Resuelto en la Fase 5 (`a18dbc2`, `e206f87`, `0aba1f1`, `8cb31a6`, `e5f35e9`): ciclo de vida con `CLOSING_REVIEW`, congelación en el repositorio, sección "Exportar y respaldar" con PDF, XLSX, CSV, respaldo y copia automática de Android. Dos cosas que el diagnóstico no preveía: las quincenas que el rollover cerraba en automático tenían los totales reales en cero y hundían la tendencia (saneadas con un barrido único), y las quincenas creadas por el rollover nunca llegaban a la nube. | `ui/profile/ProfileScreen.kt:59`; `grep` de export/closeQuincena vacío | 5 |
...
| 26 | D | **Cerrado a medias (2026-09-16; corregido el mismo día).** Cero U+2014 es regla invariable y el repositorio llegó a versionar 1395 ocurrencias en 206 archivos. La Fase 6b barrió **el código y los textos visibles**, que hoy están limpios (`app/`, `wear/`, `wearcore/`, `res/`) y protegidos por el hook `pre-commit`. La afirmación de que no quedaba ninguna en todo el repositorio era falsa: `git grep` sobre `HEAD` encuentra **930 líneas en 61 archivos** (documentación de la raíz, specs, `web/` y scripts). Ese barrido sigue siendo de la Fase 7, con la CI que lo verifique. La Fase 7 (2026-09-17) limpió `web/src`, `app/schemas/README.md` y `scripts/check_seed_integrity.sh` y puso la verificación en CI (`scripts/ci/check_em_dash.sh`: el diff no introduce ninguna y 18 rutas de código siguen en cero). El mismo día, por decisión de Agustín, se barrieron también la documentación, las specs, los scripts y las skills vendorizadas bajo `.claude/skills/` (876 líneas en 41 archivos), y el script exige cero en todo el árbol versionado. **Cerrado.** | `git grep` del 2026-09-17 | 6, 7 |
...
| 3 | Wear OS terminado y verificado | 2 | 1 | **hecha** (2026-09-07; el último pendiente, ver las dos *complications* puestas sobre una carátula, se cerró el 2026-09-17 con una carátula de prueba en el emulador) | `731a2e4` (último de contenido; el cierre documental es el commit siguiente, que actualiza esta tabla) |
| 4 | IA on-device terminada (provisión del modelo, latencia, chat) | 2 | 1 | **hecha** (2026-09-17): LiteRT-LM medido en el Pixel 7 (E2B 41 a 48 s, E4B 112 s por análisis abierto) y AICore confirmado como motor activo en el Pixel 10. **Con una salvedad que no cierra**: la descarga in-app no se puede verificar porque el proyecto Firebase **no tiene ningún bucket de Storage** (decisión 4 de §7, sigue abierta); el modelo se provisionó por `adb`. | `0fdce65` (descarga del modelo, espera del asistente, límites del chat, golden suite); la medición es del cierre documental de este día |
...
| 6 | Mejora de UX con `/design` y accesibilidad | 1, 5 | 2 (6a auditoría y diseño; 6b implementación) | **hecha** (2026-09-16; la pasada de accesibilidad en hardware se hizo el 2026-09-17 en el Pixel 7 con Accessibility Scanner y TalkBack, con once correcciones, y el recorrido de la notificación quedó auditado). Sin publicar el sistema de diseño en la nube (`/design-sync` exige `/design-login`, que solo se concede desde una sesión interactiva). **Quedan dos recorridos sin auditar por acciones que solo Agustín puede hacer**: importar el estado del mes (pegar la clave de NVIDIA en Perfil del Pixel 7) y proponer desde la web (elegir la cuenta de pruebas en la ventana de Google); el detalle está en `ui_reference/design_2026-09/AUDITORIA.md` §8 | `5859d87` (último de contenido; el cierre documental es el commit siguiente, que actualiza esta tabla) |
| 7 | Calidad: pruebas, CI y observabilidad | 2, 5 | 1 | **hecha** (2026-09-17): 91 pruebas JVM, 10 instrumentadas (migraciones sobre el asset real desde v1, v16, v17 y v18, verificadas en el Pixel 7), 395 casos de reglas, 21 de la web, CI en `ci.yml` e `instrumented.yml`, Crashlytics (decisión 7) y el repositorio entero sin rayas largas, skills vendorizadas incluidas | (el cierre documental es el commit que actualiza esta tabla) |
...
**Registro de cierre (2026-09-07).** Commits en `develop`: `d0c59be` (el hogar entra al sync), `12be51b` (lápidas para toda entidad con borrado real), `dbaa908` (ancla del saldo y aviso de divergencia), `230e124` (el sembrado de estados también re-ancla), `ff0efb2` (`statement_import` sincronizado), `19fafeb` (identidad en Perfil), `c5116bd` (grupo nuevo con categorías y cuentas), `d740eaf` (reglas por colección), `ca02f6a` (el pull del hogar actualiza en vez de reemplazar), más el commit de esta actualización. Base en **v21**.

Decisiones tomadas al arrancar la fase: (3) conservar el snapshot del saldo y añadir detección de divergencia, en vez de rediseñar a deltas idempotentes; (4) sincronizar `statement_import`; (5) el Colaborador se queda con lo mínimo para proponer, sin documento `summary` agregado. Las tres quedan razonadas en §7.

Reglas desplegadas a producción el 2026-09-07 con autorización explícita (ruleset `d9623916-085a-4eb3-81b3-fe06b0841ee5`; el anterior, `bb575318-d051-4aaa-866e-fc256a1e3cd5`, sigue en el proyecto por si hay que revertir).

Verificación: matriz de convergencia por entidad en la memoria `finance-app-fase-2-sync-identidad-reglas`. Dos limitaciones que conviene tener presentes: la prueba con una cuenta Colaborador REAL en la web no se hizo, porque exigía escribir una contraseña, y se sustituyó por 60 casos contra el emulador de reglas más la comprobación de que el push de los dos dispositivos reales sigue drenando sin rechazos; y el borrado de miembro y de categoría no tiene hoy ninguna superficie que lo dispare, ni en la app ni en la web, así que su lápida quedó verificada por el lado del pull y no por un borrado real desde la interfaz.
...
1. **El renderizado de las dos *complications* sobre una carátula.** Retomado el 2026-09-16 (`0e4a8da`). Colocarlas a mano no se puede automatizar, y ahora se sabe por qué con precisión: el emulador solo trae carátulas declarativas (WFF), su selector de complications vive dentro de sysui sin ninguna actividad que responda a la acción de elección (`pm query-activities` no devuelve nada), y los huecos los dibuja la propia carátula sin exponer nodos de accesibilidad. En su lugar se verifica el dato que una carátula recibiría, con doce casos instrumentados sobre los dos servicios (`wear/src/androidTest/`, las primeras pruebas del módulo), que de paso destaparon un defecto real: `relativeDue` contaba días transcurridos en vez de días de calendario y anunciaba como "hoy" un pago que vencía mañana. **Cerrado el 2026-09-17 sin el Pixel Watch 4:** una carátula mínima en Watch Face Format (`ui_reference/design_2026-09/wff_complications/`, con su README) declara los dos huecos con los dos servicios como proveedores por defecto, así que el sistema los coloca solo. Instalada en `FinanceWatch` emparejado con el Pixel 7 y puesta como carátula activa por `DEBUG_SURFACE`, muestra "Disponible" como arco con "$53k · Saldo" y "Próximo pago" como "$899 · hoy" con los datos reales del teléfono (captura `capturas/2026-09-17_reloj_complications_en_caratula.png`). Lo que sigue sin verse es la carátula de fábrica del Pixel Watch 4, que no cambia lo que el servicio entrega.
...
**Registro de verificación en dispositivo (2026-09-17).** Los archivos `gemma-4-E2B-it.litertlm` (2.6 GB) y `gemma-4-E4B-it.litertlm` (3.7 GB) se bajaron de los repos `litert-community` de Hugging Face, que **no están cerrados** (no piden token ni licencia, al contrario de lo que asume `scripts/admin/upload_model.py`), y se empujaron por `adb` a `getExternalFilesDir()` del Pixel 7. Medido con la traza `AiAssistant: respuesta` (nueva en este cierre) sobre la pregunta "Analiza cómo va el gasto de esta quincena y qué llama la atención", con el badge "Análisis IA" en pantalla:

| Aparato y motor | Análisis abierto | Carga del motor | Memoria del proceso |
|---|---|---|---|
| Pixel 7, LiteRT-LM, Gemma **E2B** (la de fábrica) | **48 s y 41 s** (dos muestras) | ~20 s | 2.7 GB PSS |
| Pixel 7, LiteRT-LM, Gemma **E4B** | **112 s** (una muestra); una pregunta por la ruta de intents tardó 75 s y acabó en `Unknown` | ~27 s | 3.6 GB PSS |
| Pixel 10 Pro XL, AICore | motor `AICORE` confirmado en el log al arrancar; la respuesta con LLM no se cronometró porque el teléfono se bloqueó (una respuesta determinista salió en 87 ms) | | |

La decisión de la Fase 4 se sostiene: E2B de fábrica. La cifra de 54 s del Pixel 9 se retira; en el Fold de Norma (Tensor G4) lo esperable está entre el Pixel 7 y aquel Pixel 9. Dos observaciones de calidad, no de latencia: E2B escribió "déficit proyectado de 2930" cuando el ingreso supera al gasto en esa cifra, y ambas variantes filtran códigos de categoría (`FOOD.COMIDA`) en la redacción; son del prompt y quedan anotadas para la Fase 7 o el cutover.

**Lo que no cierra y por qué.** El proyecto `finance-app-abdf9` **no tiene ningún bucket de Cloud Storage** (`list_buckets` devuelve vacío; los dos nombres candidatos dan 404), así que `ModelDownloader` no tiene de dónde bajar y la descarga in-app no se puede verificar. Crear el bucket es la decisión 4 de §7 (Firebase Storage exige el plan Blaze desde 2024) y le corresponde a Agustín; una vez creado, `upload_model.py --variant E2B` con el archivo ya bajado en esta sesión publica el modelo y el manifiesto.
...
**Cerrada el 2026-09-15** (`e5f35e9`). Lo entregado y lo que cambió respecto a lo planeado:

- Al vencer, la quincena queda en `CLOSING_REVIEW` ("pendiente de cierre") y sigue editable; el cierre lo hace una persona. Antes el rollover cerraba solo y nadie revisaba lo planeado sin ejecutar.
- La congelación vive en el repositorio, no en la interfaz, así que cubre captura, detalle, libro mayor y recordatorios de una vez. Hizo falta una segunda instancia del repositorio de gastos **sin** guardia para los procesos del sistema (sembrado de estados, aplicación de un estado de cuenta, curación de plantillas), que escriben sobre periodos históricos ya cerrados.
- El ciclo de vida entra al sync con el kind `QUINCENA`. De paso se arregló que las quincenas creadas por el rollover nunca llegaban a la nube.
- Barrido único de arranque: las quincenas que el rollover había cerrado en automático tenían `actual_income_mxn` y `actual_expenses_mxn` en cero y hundían la tendencia de Analíticas. Se recalculan **sin** sellar `updated_at`, para no fabricar una marca más nueva que el cierre de otro dispositivo.
- Exportación: PDF con `PdfDocument`, libro XLSX con escritor propio (se descartó `fastexcel`: añade dependencias sin aportar al layout) y CSV por periodo. Respaldo con `VACUUM INTO`, restauración validada con reinicio de proceso y copia automática de Android activada con lista blanca.
- **Abierto:** la web no aplica la congelación (decisión 12 de §7).
...
**Estado al 2026-09-16: la fase está cerrada.** La 6a dejó la auditoría medida (`ui_reference/design_2026-09/AUDITORIA.md`), `DESIGN.md` reescrito con los tokens del código, los cinco briefs (`BRIEFS.md`) y el paquete de vistas previas en `design_system/`. Lo único que no se hizo es publicar ese sistema de diseño en la nube: `/design-sync` exige `/design-login` y esa autorización solo se concede desde una sesión interactiva; los lienzos viven en `artboards/` y no dependen de la nube.

La 6b cerró primero lo que no dependía de un lienzo (los tres objetivos táctiles reales, el rol de botón en los clicables hechos a mano, la región viva del guardado, `strings.xml`, el barrido de rayas largas y **Quick Tap completo**) y el 2026-09-16 los cuatro rediseños, con la opción elegida por Agustín anotada en `DECISIONES.md`:

1. **Hoja de captura (A + C).** El teclado propio ocupa alto solo mientras se teclea el importe: se retira al tocar otra cosa o al desplazar, y vuelve al tocar la cifra. Arriba, "Las de siempre, de un toque" ofrece hasta tres gastos que el hogar repite, completos, del mismo motor que el panel de Quick Tap: tocarlos guarda el gasto entero.
2. **Perfil.** Cuatro grupos plegables (Este mes, El hogar, Apariencia e inteligencia, Automatización y avisos), uno abierto a la vez, con la ayuda y el estado de la sesión fuera de los grupos. Las cabeceras son encabezados de verdad para TalkBack y dicen si están abiertas.
3. **Bloque héroe.** El anillo gana un segundo tramo con lo reservado y debajo va la resta escrita (entró, menos gastado, menos reservado, igual a disponible); los tres tiles de colores se retiran. Los términos se redondean antes de restarse, así que la línea cuadra siempre.
4. **Cierre de quincena.** Cabecera fija con la decisión masiva y el conteo de lo que falta por decidir, y barra inferior siempre visible con el resumen de lo que se congela y el botón de cerrar.

Todo verificado en FinanceFold con la configuración fiel del Fold (2076×2152, densidad 408, fuente 1.3 con negrita, modo oscuro); las capturas del 2026-09-16 están en `ui_reference/design_2026-09/capturas/`. Queda abierto, y no es de esta fase, el arranque en frío de 2.3 s con el proceso muerto (hallazgo 7bis.1: toca a `BudgetApplication.onCreate`, no a Quick Tap) y la pasada con TalkBack y Accessibility Scanner en hardware, que es lo único que mide contraste real y orden de foco.
...
**Resultado (2026-09-17).** Los seis puntos hechos.

- *Pruebas JVM* (`app/src/test/`, 91 casos): `InstallmentSchedule` (11), `StatementCycleTracker` (9), `RecurrenceDetector` (11), `RecurrenceMaterializer` (12, ids deterministas y particiones de 10,000 bps con los DAO simulados por MockK), `JsonRepairer` (9), `QuestionClassifier` (8), `MoneyFormat` (4), `AttributionShares` (8: la conversión de porcentaje a basis points salió de `CaptureViewModel` a un objeto puro para poder probarla), `QuincenaFigures` (6) y la golden suite (5: la heurística de intents contra `AliasResolver` cargado desde la semilla real por JDBC, el análisis abierto y el alcance del asistente). `AmountSemantics` solo existe en composición, así que se prueba instrumentada.
- *Instrumentadas* (`app/src/androidTest/`, 10 casos, verificadas en el Pixel 7 con Android 16): la cadena 1→21 sobre el asset real sin perder filas en las doce tablas sembradas, las rutas desde v16, v17 (construida a mano porque no hay `17.json`) y v18, la idempotencia de la cola de la cadena sobre una base ya migrada (el escenario de las ramas divergentes), la migración de datos 19→20 y el esquema v1 vacío; más las señales no cromáticas del tema en claro, oscuro y color dinámico. Runner propio (`PlainAppRunner`) para no arrancar `BudgetApplication`; el tema se compone con `ActivityScenario` y sin Espresso, porque Espresso (3.5.0 y 3.6.1) revienta en Android 16 con `InputManager.getInstance`.
- *Reglas* (`scripts/rules/`, 395 casos): matriz de cinco identidades por doce colecciones por cinco operaciones, más hogar, roles, invitaciones, propuestas, `users` e `invite_codes`. El Colaborador no lee ninguna colección del ledger ni sus atribuciones.
- *Web* (Vitest, 21 casos): `pick` dual, espejo snake_case, los ocho normalizadores de documentos, lápidas, fechas en zona de México, código de invitación legacy y las rutas por rol (`web/src/lib/access.ts`, que ahora alimenta al guard y a la barra).
- *CI*: `.github/workflows/ci.yml` (cero U+2014, compilación de `:app` y `:wear`, pruebas JVM, integridad de la semilla, web y reglas) e `instrumented.yml` (manual, teléfono API 34 y Wear API 33 con `am instrument`). `scripts/ci/run_instrumented.sh` sirve igual en CI y en esta máquina.
- *Observabilidad*: Crashlytics, por decisión de Agustín del mismo día (§7, decisión 7). Plugin y SDK en `:app`, sin código; símbolos de release en la Fase 8.
- *Rayas largas*: por decisión de Agustín del mismo día, el barrido cubrió todo el repositorio, skills vendorizadas incluidas, y `check_em_dash.sh` exige cero en todo el árbol versionado. Nota para la Fase 10: la nota del préstamo que el ETL escribe en `loan.notes` (`Deben $3,600: Medicina + rotafolio`) perdió su raya en el script, así que el golden vigente, que la conserva, difiere de lo que produciría una regeneración; el reseed de la Fase 10 promueve un golden nuevo y lo absorbe.
...
7. **Observabilidad.** Resuelta el 2026-09-17: **Crashlytics**. Entra en `:app` por el BOM de Firebase y el plugin de Gradle, sin código; la subida de símbolos de release queda para la Fase 8 con la minificación. Se descartó el registro local exportable.
...
9. **Deriva de saldo.** Resuelta el 2026-09-07: se conserva el snapshot con LWW y se añade detección de divergencia. `opening_balance_mxn` pasa a ser el ancla real y `balance_anchor_at` (v21) marca hasta cuándo los movimientos ya están contenidos en él; el saldo derivado se calcula por SQL y, si difiere más de medio peso, la tarjeta de la cuenta avisa y la conciliación ofrece el saldo calculado. El rediseño a deltas idempotentes por movimiento se descartó: exigía migración de datos, reescribir la aplicación de estados de cuenta y revisar todo lo que lee el saldo, a cambio de prevenir una deriva que esta forma hace visible y corregible en un toque.
10. **`statement_import` y el checklist mensual.** Resuelta el 2026-09-07: se sincroniza (kind STATEMENT, v21), porque Agustín y Norma comparten el seguimiento. `payload_json` no viaja: el checklist, la tarjeta de deuda y el recordatorio de pago solo necesitan la cabecera, y el texto crudo del LLM solo le sirve al dispositivo que importó el PDF.
12. **Congelación en la web.** Abierta desde 2026-09-15. La app rechaza altas y ediciones sobre una quincena cerrada dentro de la transacción del repositorio, pero la PWA (`web/src/lib/repository.ts`) edita cualquier periodo sin mirar su estado. Poner el mismo guard allí exige tocar la web y volver a desplegar hosting; mientras no se haga, quien use el escritorio puede modificar un periodo cerrado y el cambio llegará al teléfono por el pull, que escribe por DAO directo y no pasa por el guard. Se decide cuando la Fase 8 vuelva a tocar la web.

11. **Alcance de lectura del Colaborador.** Resuelta el 2026-09-07: lee `members`, `categories`, `wallets` y `proposals`, que es justo lo que necesita para proponer y lo que su web consume. Se descartó el documento agregado `summary/{quincenaId}`: obligaba a decidir quién lo escribe y cuándo, con riesgo de quedar rancio si nadie abre la app, añadía una escritura más al push y exigía adaptar la web, a cambio de un panorama que el Colaborador no ha pedido.
