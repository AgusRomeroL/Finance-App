...
/**
 * Implementación Room del [InstallmentRepository] — MVP Fase 3.
 * Local-only por ahora (sin outbox de sync; Fase 3.5 lo añadiría siguiendo el
 * patrón TRANSFER con migración de `updated_at`).
 */
class InstallmentRepositoryImpl(
    ...
    override suspend fun insert(plan: InstallmentPlanEntity) = dao.insert(plan)

    override suspend fun update(plan: InstallmentPlanEntity) = dao.update(plan)
