...
**Fase 4 (UI Calendario) COMPLETA** (inc. 1 timeline + 2a mes Google-Calendar + 2b pago manual + 2c CRUD plantillas + 2d preset lead). **SIGUIENTE = Fase 5** (inferencia bajo autorización: detector sobre recurrencia → propone crear plantilla, propose-then-confirm) → luego Fase 6 (espejo Google Calendar opt-in una vía). Pendientes menores abiertos: budget-awareness reservado-vs-restado (§G.2.4), "saltar/editar esta ocurrencia" sin tocar la plantilla (§G.2.6), CUSTOM_CRON diferido.
