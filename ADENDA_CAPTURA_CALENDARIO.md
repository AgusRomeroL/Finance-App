...
**SIGUIENTE = Fase 6** (espejo Google Calendar opt-in, una vía: toggle en Perfil + `WRITE_CALENDAR` solo si se activa, `CalendarContract` writer best-effort, guardar `external_event_id`). Pendientes menores abiertos: budget-awareness reservado-vs-restado (§G.2.4), "saltar/editar esta ocurrencia" sin tocar la plantilla (§G.2.6), CUSTOM_CRON diferido.
