...
- **Fase 3: Recordatorios (WorkManager) HECHA, verificada en emulador y commiteada (`220ceb6`).** Paquete nuevo `data/reminder/`:
  - `ReminderWorker` (`PeriodicWorkRequest`, piso 15 min, **NO** `SCHEDULE_EXACT_ALARM`). Agendado en `BudgetApplication.scheduleReminders()` con `enqueueUniquePeriodicWork(KEEP)`. En cada corrida lee los PLANNED del hogar (`ExpenseDao.getPlannedForReminder`: LEFT JOIN a `recurrence_template` por su `cadence_detail` + INNER JOIN a `quincena` por `start_date`), resuelve el lead, notifica los vencidos elegibles.
  - **Resolución de lead:** override por plantilla en `cadence_detail` (`reminder_lead_days` entero **o** `"QUINCENA_START"`) → si no, default global `SettingsRepository.reminderLeadDays` (2 días). `QUINCENA_START` dispara al inicio de la quincena (parse ISO `start_date` en `America/Mexico_City`).
  - **Estado anti-renag (sin migración):** mapa `expenseId → próximo-elegible (epoch millis)` en DataStore (`reminder_state_json`). Al notificar se fija `NEVER_AGAIN` (Long.MAX); el worker **poda** el mapa a los PLANNED vigentes cada corrida (confirmar→POSTED o borrar lo saca solo).
  - **Notificación** `ReminderNotifier` (canal propio `planned_reminders`) con 3 acciones: **Confirmar** → `ReminderActionReceiver` → `expenseRepository.confirmPlanned` (POSTED, monto previsto); **Posponer** → snooze 1 día (`SettingsRepository.snoozeReminder`, sigue PLANNED); **Editar** → abre `MainActivity` (la edición fina de monto/atribución vive en dashboard/calendario, Fase 4). Receiver declarado en el manifest.
  - **Verificación (emulador FinanceFold, API 34, 2026-06-28):** sembrados 4 PLANNED cubriendo cada rama y forzado el worker (trigger one-time temporal, patrón Fase 1/2, ya revertido). Resultados: (A) sin plantilla, lead global 2d, occurred hoy → **NOTIFICADO**; (B) futuro 07-10 → omitido (no due); (C) plantilla `QUINCENA_START`, occurred futuro 07-10 → **NOTIFICADO** (dispara al inicio de quincena 06-16, no a la fecha del gasto); (D) plantilla `reminder_lead_days:0`, occurred 07-01 → omitido (el override suprime lo que el lead global 2d **sí** dispararía). 2 notificaciones posteadas con formato `$8,500.00`/`$3,000.00` y 3 acciones c/u. **Confirmar** (tap real del PendingIntent en la sombra) → rem-A POSTED + `sync_queue` EXPENSE/UPSERT + notif cancelada. **Posponer** → snooze exacto now+1d, notif cancelada, sigue PLANNED; re-corrida del worker NO re-notifica (poda rem-A ya POSTED, respeta el snooze de rem-C). Receiver `exported=false` correctamente rechaza `am broadcast` desde shell.
  - **Deuda honesta:** (a) "Editar" solo navega a la app, no abre aún un editor del PLANNED puntual (espera Fase 4). (b) Preset UI del lead ("mismo día / 1 / 2 / 3 / 7 / al inicio de la quincena") **no** existe todavía: el lead global se setea por API, no hay control en Perfil.

- **Fase 4: UI de Calendario, INCREMENTO 1 (timeline de PLANNED) IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear).** Paquete nuevo `ui/calendar/`:
  - **Destino de navegación nuevo `CALENDAR` ("calendar")**, ocupa el slot de nav que antes era el placeholder "Libro" (`navItems` en `DashboardScreen`: ícono `CalendarMonth`, label "Calendario"). El placeholder `LEDGER` queda en el grafo pero ya no está en el rail/bottom-nav.
  - `CalendarViewModel` observa `ExpenseDao.observePlannedWithDetails(householdId)` (PLANNED del hogar con detalles, `ORDER BY occurred_at ASC`; query nueva, sin migración). `CalendarScreen` = timeline con tarjeta por gasto (ícono de categoría reusando `iconForCategory`, concepto, fecha `EEE d MMM`, monto MXN) + 3 acciones por fila.
  - **Acciones reusan Fases 2/3:** **Confirmar** → `confirmPlanned(id)` (POSTED, monto previsto); **Editar** → `AlertDialog` con el monto prefijado → `confirmPlanned(id, montoReal)` (recurrentes que varían: luz/agua); **Posponer** → `snoozeReminder(now+1d)` + snackbar (sigue PLANNED). Toda acción cancela la notificación de recordatorio pendiente (`ReminderNotifier.cancel`). VM cableado en `MainActivity` (`CalendarViewModelFactory`).
  - **Animaciones Material Expressive:** `Modifier.animateItem` con specs de resorte (aparición/reordenamiento/descarte de filas).
  - **Verificación (FinanceFold API 34, layout expandido/rail):** 3 PLANNED en timeline ordenado por fecha. Confirmar simple → POSTED monto previsto + sale de la lista. Editar 600→**750** → POSTED 750 + `sync_queue` UPSERT. Posponer → snackbar "Recordatorio pospuesto", sigue PLANNED, snooze persistido. Header reactivo "3→2→1 planeados".
- **Fase 4 inc. 2a: VISTA DE MES estilo Google Calendar IMPLEMENTADA y VERIFICADA en emulador (pendiente commitear).** `ui/calendar/MonthCalendar.kt`: cuadrícula 6×7 lunes-primero con navegación de mes (`YearMonth` ± 1, label `LLLL yyyy` con `AnimatedContent`), cabecera L-M-M-J-V-S-D, **día de hoy con anillo**, **día seleccionado con círculo relleno**, **punto marcador en los días con `PLANNED`** (mapeo `occurred_at`→`LocalDate` en `America/Mexico_City`, consistente con cómo el materializer guarda 9:00 MX). En `CalendarScreen` el mes va como encabezado del `LazyColumn` (scrollea con la lista); tocar un día **filtra** la lista a ese día (header → "Pagos: Sábado 20 de junio") y tocarlo de nuevo limpia el filtro. **Verif (FinanceFold API 34):** 5 PLANNED en junio (días 16/20/25/30) → puntos exactos en esos días, anillo en hoy (28); tap día 20 → lista filtra a Luz CFE + Agua SIAPA; tap de nuevo → "5 planeados" completo; navegación ›› → "Agosto 2026".
  - **Pendiente (incrementos 2b+ de Fase 4):** gestión de plantillas recurrentes (CRUD: crear/editar con selector de cadencia/monto/fechas/lead + `AttributionShareEditor` compartido; pausar/reanudar/eliminar); crear PLANNED manual one-off (encaja con un FAB sobre el día seleccionado del calendario); **preset UI del lead** de recordatorio (Perfil/plantilla, deuda de Fase 3); budget-awareness (reservado vs restado, §G.2.4).

- **Fase 4 inc. 2b: PAGO MANUAL one-off (PLANNED) IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear).** `ui/calendar/NewPlanned{ViewModel,Sheet}.kt`:
  - **FAB** en `CalendarScreen` → `start(selectedDay ?: hoy)` + `ModalBottomSheet` con el formulario completo: concepto, monto, categoría/método de pago (`ExposedDropdownMenuBox`), **fecha** (`DatePicker` M3, default = día seleccionado del calendario) y atribución beneficiario/pagador **reusando `AttributionDimension`** (el control compartido con captura/revisión).
  - `NewPlannedViewModel` (VM retenido, `start(date)` reinicia el form): defaults beneficiarios=todos por igual, pagador=dueño del primer wallet; `save()` valida (sumas=100), resuelve la quincena de la fecha (**`QuincenaDao.getForDate`**, query nueva sin migración; fallback a activa), construye `ExpenseEntity(status=PLANNED, occurred_at=9:00 MX)` + atribuciones (%→bps, último absorbe el resto) e `insertWithAttributions`. Cableado en `MainActivity` (`NewPlannedViewModelFactory`)+`BudgetNavGraph`.
  - **Verif (FinanceFold API 34):** FAB → form con defaults correctos (6 miembros 100%, pagador=dueño del wallet); concepto "Tenencia auto" $2500 cat. Diversión fecha 11 jun → guardar → PLANNED en DB, atribuciones 10000/rol, sync UPSERT, **`getForDate` asignó la quincena del 11 jun (Q1, distinta de la activa Q2)**; aparece en el calendario (día 11) y en la lista filtrada con sus acciones.

- **Fase 4 inc. 2c: CRUD de plantillas recurrentes IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear).** `ui/calendar/{RecurrenceViewModel,TemplatesScreen}.kt`:
  - **Destino de navegación nuevo `TEMPLATES`**, accesible desde un **icono Repeat en el header del Calendario**. `TemplatesScreen` = lista de plantillas (activas + pausadas vía `RecurrenceRepository.observeAll`, query/método nuevos sin migración) con concepto, monto·cadencia, chip "Pausada", y acciones **Pausar/Reanudar** + **Eliminar** (con `AlertDialog` de confirmación). FAB → editor.
  - **Editor** (`ModalBottomSheet`): concepto, monto default, categoría/método (`DropdownField` compartido), **cadencia** (enum `Cadence`: QUINCENAL_FIRST/SECOND/EVERY, MONTHLY_SPECIFIC_HALF, BIMONTHLY), día del mes, **lead** (chips: mismo día/1/2/3/7 + **"Inicio de quincena"**), y splits beneficiario/pagador reusando `AttributionDimension`.
  - **Serialización compatible con el materializer:** `cadence_detail` `{day_of_month, reminder_lead_days(int|"QUINCENA_START")}`; `default_payer_split` `{id:bps}`; `default_beneficiary_ids` ahora `{id:bps}` (% personalizado); para ello se **extendió `RecurrenceMaterializer.parseBeneficiaries`** para aceptar objeto-o-array (aditivo, retrocompatible con el array histórico). Tras guardar, **materializa la quincena activa** (idempotente) para reflejar el PLANNED de inmediato. VM cableado en `MainActivity`+`BudgetNavGraph`.
  - **Verif (FinanceFold API 34):** crear "Gimnasio" $800 QUINCENAL_SECOND día 20 lead 2d → plantilla en DB con JSON correcto (beneficiarios {id:bps} 6 miembros, payer {id:10000}) + **PLANNED materializado** Gimnasio $800 el 20-jun con `recurrence_template_id`. Pausar → `is_active=0` + "Pausada"/"Reanudar". Eliminar → confirma → plantilla borrada, el PLANNED ya materializado se conserva (FK `SET_NULL`).

- **Fase 4 inc. 2d: PRESET DE LEAD en Perfil IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear).** Tarjeta "RECORDATORIOS" en `ProfileScreen` con un `SettingRow` "Antelación de recordatorios" cuyo subtítulo refleja el lead actual; al tocar abre un `AlertDialog` con radios (mismo día/1/2/3/7) → `SettingsRepository.setReminderLeadDays`. Cableado `reminderLeadDays` (collectAsState) + setter por `BudgetNavGraph`/`MainActivity`. **Verif (FinanceFold API 34):** default "2 días antes"; seleccionar "1 día antes" → persiste en DataStore y el subtítulo reacciona; el `ReminderWorker` lee el mismo flujo, así que lo respeta. (La opción `QUINCENA_START` vive como override por plantilla en el editor de inc. 2c, no como global.)

**Fase 4 (UI Calendario) COMPLETA** (inc. 1 timeline + 2a mes Google-Calendar + 2b pago manual + 2c CRUD plantillas + 2d preset lead).

- **Fase 5: INFERENCIA BAJO AUTORIZACIÓN IMPLEMENTADA y VERIFICADA en emulador con datos reales (pendiente commitear).** `data/recurrence/RecurrenceDetector.kt` (motor puro determinista): agrupa el historial POSTED por `concept_canonical` (el de Feature B) y mide regularidad → confianza `0.4·sigmoid(n-3) + 0.3·(1-interval_cv) + 0.3·(1-amount_cv)` (cv acotado); infiere cadencia por intervalo medio (~15d→quincenal con mitad por día modal/EVERY; ~30d→mensual; ~60d→bimestral), día modal, monto mediano; filtra por umbral 0.55. **`RecurrenceViewModel`** expone `suggestions` (combine de `observeAll`+descartes+miembros → recarga historial → detecta → filtra lo ya plantillado por clave canónica y lo descartado). **`acceptSuggestion`** (propose-then-confirm, ÚNICA vía de creación): infiere splits beneficiario/pagador con `RetroAttributionEngine.suggestForKey` (fallback equalSplit/dueño-wallet), guarda `confidence_score` + `learned_from_expense_ids` (JSON), inserta y materializa la quincena activa. **`dismissSuggestion`** persiste la clave canónica en DataStore (`dismissed_template_suggestions`, nuevo stringSet) → no vuelve a proponerse. En `TemplatesScreen`, sección "SUGERIDAS POR TU HISTORIAL" (tarjetas con `AutoAwesome`, reason "≈$X · cadencia · N registros", [Crear plantilla]/[Descartar]) arriba de "TUS PLANTILLAS".
  - **Verif (FinanceFold API 34, semilla real 793 gastos):** detectó Adobe/KIGO/Youtube ("cada mes"), Comida Gatas/Internet, etc. Crear Adobe → plantilla MONTHLY día 20 con **confidence 0.992**, `learned_from_expense_ids` poblado, splits inferidos {miembro:10000} ambos roles, + PLANNED materializado el 20-jun; su sugerencia desapareció y subió la siguiente. Descartar KIGO → "kigo" en `dismissed_template_suggestions` (DataStore) y desapareció. Nada se creó sin "Crear plantilla" (propose-then-confirm).

- **Fase 6: ESPEJO GOOGLE CALENDAR (opt-in, una vía) IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear). ÚLTIMA fase del bloque G.** `data/calendar/CalendarMirror.kt`: writer best-effort a `CalendarContract` hacia un **calendario dedicado LOCAL "Presupuesto Familiar"** (cuenta `ACCOUNT_TYPE_LOCAL`); nunca lee ni toca los calendarios personales. `reconcile()` idempotente: inserta los PLANNED sin evento y borra los eventos cuyos PLANNED ya no existen; guarda `expenseId→eventId` en DataStore (`calendar_event_map_json`, una vía para update/delete). `disableAndPurge()` borra eventos + calendario y limpia el estado. Toggle "Espejo en Google Calendar" en Perfil (tarjeta CALENDARIO) → al activar pide `READ/WRITE_CALENDAR` en runtime (`RequestMultiplePermissions` en `MainActivity`); al conceder activa + reconcilia; al desactivar purga. Permisos declarados en manifest. Reconcilia también al arrancar (`BudgetApplication`) si está activo. Todo en `runCatching` (si falta permiso/proveedor, la app sigue normal). **De paso se arregló un bug:** `ProfileScreen` no tenía `verticalScroll` → con la tarjeta nueva las últimas quedaban fuera de pantalla; se añadió scroll.
  - **Verif (FinanceFold API 34):** 2 PLANNED sembrados → toggle ON → diálogo de permiso → Allow → calendario "Presupuesto Familiar" creado + eventos "$8,500.00 · Renta" (20-jun 9:00 MX) y "$899.00 · Internet" (25-jun) en el `CalendarProvider` (verificado por `content query`). Toggle OFF → eventos + calendario borrados, `calendar_event_map_json` = `{}`. Una vía, dedicado, opt-in.
  - **Deuda honesta:** la reconciliación corre al activar y al arrancar la app; un PLANNED creado **después** aparece en el calendario hasta el siguiente arranque (best-effort one-way; suficiente para el caso de uso). No actualiza el evento si cambia el monto del PLANNED (lo recrea solo si el id cambia).

**BLOQUE G (captura/calendario) COMPLETO:** G.1 bandeja unificada + Fases 0-6 (recurrencia → materialización → confirmación → recordatorios → UI calendario completa → inferencia bajo autorización → espejo Google Calendar). Pendientes menores abiertos (no bloquean): budget-awareness reservado-vs-restado (§G.2.4), "saltar/editar esta ocurrencia" sin tocar la plantilla (§G.2.6), CUSTOM_CRON diferido, captura NL voz/widget/reloj (G.3, exige habilitar módulo `wear/`).
...
| **Ninguna** | ninguno | Cero ubicación. |
