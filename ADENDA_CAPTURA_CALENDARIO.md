...
- **Fase 4 — UI de Calendario, INCREMENTO 1 (timeline de PLANNED) IMPLEMENTADO y VERIFICADO en emulador (pendiente commitear).** Paquete nuevo `ui/calendar/`:
  ...
  - **Pendiente (incrementos 2+ de Fase 4):** gestión de plantillas recurrentes (CRUD: crear/editar con selector de cadencia/monto/fechas/lead + `AttributionShareEditor` compartido; pausar/reanudar/eliminar); crear PLANNED manual one-off; **preset UI del lead** de recordatorio (Perfil/plantilla — deuda de Fase 3); budget-awareness (reservado vs restado, §G.2.4).

**SIGUIENTE = Fase 4 incremento 2:** gestión de plantillas recurrentes (CRUD) + PLANNED manual + preset de lead. Luego Fases 5 (inferencia bajo autorización) → 6 (espejo Google Calendar).
