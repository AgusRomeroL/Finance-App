...
- **Fase 3 — Recordatorios (WorkManager) IMPLEMENTADA y VERIFICADA en emulador (pendiente commitear).** Paquete nuevo `data/reminder/`:
...
**SIGUIENTE = Fase 4 (§G.2.2):** UI de Calendario (destino `CALENDAR` reusando el placeholder `LEDGER`): timeline de PLANNED + gestión de plantillas (`AttributionShareEditor` compartido) + preset UI del lead de recordatorio en Perfil/plantilla. Luego Fases 5 (inferencia bajo autorización) → 6 (espejo Google Calendar).
