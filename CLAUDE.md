...
> **Trabajo grande del 2026-06-28 (ya commiteado en `main`):** (1) **Capa de IA proactiva B→A→C→D + Capa 3 LLM** completa y la Capa 3 validada en hardware real (híbrido AICore/LiteRT-LM — ver `ADENDA_IA_PROACTIVA.md` §F.8.4). (2) **Bloque de captura/calendario** (`ADENDA_CAPTURA_CALENDARIO.md`, Apéndice G): bandeja unificada `pending_capture` (Room v5→v6) + **calendario/recurrencia Fases 0/1/2** (gastos PLANNED materializados desde `recurrence_template`, confirmación PLANNED→POSTED). **Fase 3 (`ReminderWorker`) IMPLEMENTADA y VERIFICADA en emulador, aún SIN commitear** (paquete `data/reminder/`, ver §G.2.7); **SIGUIENTE = Fase 4** (UI calendario). (3) **Fix de UI completo** a settings reales del Pixel 9 Pro Fold de Norma (font 1.3 + bold + pantalla casi cuadrada). Detalles por feature en los memory files / adendas; abajo siguen los stubs vigentes.
